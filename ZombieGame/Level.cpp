...
Level::Level(const std::string& fileName) {
    ...
    // Error checking
    if (file.fail()) {
        WMPEngine::fatalError("Failed to open " + fileName);
    }
	...
	WMPEngine::ColorRGBA8 whiteColor;
    whiteColor.r = 255;
    whiteColor.g = 255;
    whiteColor.b = 255;
    whiteColor.a = 255;
            ...
            switch (tile) {
                ...
                case 'R':
                    _spriteBatch.draw(destRect,
                                      uvRect,
                                      WMPEngine::ResourceManager::getTexture("Textures/red_bricks.png").id,
                                      0.0f,
                                      whiteColor);      
                    break;
                case 'G':
                    _spriteBatch.draw(destRect,
                                      uvRect,
                                      WMPEngine::ResourceManager::getTexture("Textures/glass.png").id,
                                      0.0f,
                                      whiteColor);
                    break;
                case 'L':
                    _spriteBatch.draw(destRect,
                                      uvRect,
                                      WMPEngine::ResourceManager::getTexture("Textures/light_bricks.png").id,
                                      0.0f,
                                      whiteColor);
                    break;
                case '@':
